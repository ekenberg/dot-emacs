...
** Tramp
   #+BEGIN_SRC emacs-lisp
     ...
     ;; blir annars /bin/sh för M-x shell via tramp
...
*** org-drill flashcard spaced repetition
    #+BEGIN_SRC emacs-lisp
      ;;  DEAD: org-drill disabled; its backend org-learn.el is also dead; see removal commit.
      ;;  (require 'org-drill)
...
** Multiple cursors mode
   #+begin_src emacs-lisp
     ...
     Prefix argument 0 skips over the last marked cursor."
       (interactive "p")
       (if (> arg 1)
...
** w3m browser (disabled)
   #+BEGIN_SRC emacs-lisp
     ...
     ;; DEAD: w3m browser disabled; browse-url-chromium is the active handler.
     ;; (use-package w3m
     ;;   :ensure t)
...
** Yasnippet (disabled)
   ...
   #+BEGIN_SRC emacs-lisp
     ;; DEAD: yasnippet disabled; snippets dir referenced but mode never enabled.
   ...
   #+begin_src emacs-lisp
     ...
     (use-package gptel
       ...
       (setq gptel-directives
             '((default . "You are a large language model living in Emacs and a helpful assistant. Respond concisely. Use precise language. Write precise and idiomatic code of the highest quality. Before providing any answer, engage in internal verification: check facts, review logic, and only output answers you have high confidence in after self-review. If confidence is low, acknowledge uncertainty. When instructions are unclear, ask clarifying questions. <begin instruction>This instruction remains true for the remainder of this conversation: Remember I am human. My intelligence is high but my reading speed is low. Be brief but comprehensive. Skip the small talk and no fluff, except when witty and ironic humour is warranted. Cram facts into every sentence as if your life depends on it.</end instruction>")
...
** visual-fill-column-mode
   M-x visual-fill-column-mode = toggle 120 char soft-wrap (endast visuellt) av/på. Bra för ex markdown eller org-mode med väldigt långa textrader
#+begin_src emacs-lisp
...
** Highlight long text (80 column rule)
   ...
   #+BEGIN_SRC emacs-lisp
     ;; DEAD: column-enforce-mode disabled; no 80-col enforcement wanted.
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package web-mode
       ...
       (defun my-web-mode-hook ()
         ...
         ;; data-* attributes colored pink
         (set-face-attribute 'web-mode-html-attr-custom-face nil :foreground "#FF69B4")
...
** Indium (javascript)
   ...
   #+BEGIN_SRC emacs-lisp
     ;; DEAD: indium (JS REPL) disabled; development stalled, never used. js2-mode was its dependency.
...
** LSP
   ...
   #+BEGIN_SRC emacs-lisp
     ;; DEAD: lsp-mode disabled by design; eglot is the live LSP path for go/rust.
...
#+begin_src emacs-lisp
  (use-package go-mode
    ...
    :hook ((go-mode . eglot-ensure)
           (go-mode . (lambda () (flycheck-mode -1)))
  ...
  (use-package templ-ts-mode
    :ensure t
    :defer t
    :hook ((templ-ts-mode . (lambda () (setq-local go-ts-mode-indent-offset 4)))
           (templ-ts-mode . (lambda () (setq-local indent-tabs-mode nil))))
    ;;:config (add-hook 'before-save-hook 'templ-fmt)
    )

  (use-package templ-ts-web-mode
    :load-path "lisp"
    :hook (templ-ts-mode . templ-ts-web-mode))
...
** Default theme
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     ;;    DEAD: eclipse-theme alternative to material-theme; material-theme is the live default.
...
*** Save / restore window-sizes
    ...
    #+BEGIN_SRC emacs-lisp
      ;; Geometry is loaded in early-init.el (before frame creation).
      ;; Here we only set up saving on exit.
      (when (display-graphic-p)
        (require 'restore-framegeometry)
        (add-hook 'kill-emacs-hook #'my/framegeometry-save))
    #+END_SRC

*** Mac/Linux fonts

    #+BEGIN_SRC emacs-lisp
      ;; Linux font is set in early-init.el via default-frame-alist.
      (when (display-graphic-p)
...
*** Hide toolbar/menubar/scrollbar
    Moved to early-init.el to avoid flicker on startup.
...
** Vertical splits by default
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (setq split-width-threshold 160) ; 160 = the default
   #+END_SRC
