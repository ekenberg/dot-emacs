...
#+begin_src emacs-lisp
  ...
  ;; jo det funkar typ men den hittar inte "prettier" så det blir tabs = 8 spaces som indent, meh
  ;; man kan joxa med prettier och se till att den finns i path och sen anpassa options
  ;; med -prettier-command, se 'templ fmt --help' för mer detaljer
  (defun templ-fmt ()
    "Format the current buffer with templ fmt."
    (interactive)
    (let ((tmpfile (make-nearby-temp-file "templfmt" nil ".templ"))
          (coding-system-for-read 'utf-8)
          (coding-system-for-write 'utf-8))
      (unwind-protect
          (progn
            (write-region nil nil tmpfile)
            (if (zerop (call-process "templ" nil nil nil "fmt" (file-local-name tmpfile)))
                (let ((tmpbuf (generate-new-buffer " *templ-fmt*")))
                  (unwind-protect
                      (progn
